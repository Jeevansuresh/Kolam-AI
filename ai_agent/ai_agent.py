...
def fetch_recent_chats(limit=20):
    chats = session.query(ChatHistory).order_by(ChatHistory.timestamp.desc()).limit(limit).all()
    chats.reverse()
    return chats

def fetch_recent_memory(limit=10):
    mems = session.query(MemoryLog).order_by(MemoryLog.timestamp.desc()).limit(limit).all()
    mems.reverse()
    return mems

def save_chat(sender, message):
    chat = ChatHistory(sender=sender, message=message, timestamp=datetime.utcnow())
    session.add(chat)
    session.commit()

def save_memory(memory):
    mem = MemoryLog(memory=memory, timestamp=datetime.utcnow())
    session.add(mem)
    session.commit()
...
def build_memory_log():
    mems = fetch_recent_memory()
    return "\n".join([m.memory for m in mems])

def parse_response(text):
    reply, intent, item = "", "", ""
    if not text:
        return "Sorry, something went wrong.", "unknown", "none"
    ...
    return reply or "Sorry, something went wrong.", intent or "unknown", item or "none"

def generate_response(user_input):
    save_chat("guest", user_input)
    save_memory(f"Guest said: {user_input}")
...
"""
    ...
    try:
        ...
        print("Raw Gemini response:", full_reply)

        reply, intent, item = parse_response(full_reply)

        save_chat("bot", reply)
        save_memory(f"Kolamai replied: {reply}, Intent: {intent}, Item: {item}")
        ...
        save_chat("bot", error_msg)
        save_memory(f"Error: {e}")
        return error_msg
